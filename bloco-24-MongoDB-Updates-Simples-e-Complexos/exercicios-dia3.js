...
db.movies.updateOne({'title': 'Batman'}, {$addToSet: {'ratings': { $each: [85, 100, 102, 105]}}});

//? 4 - Adicione um array chamado ratings ao filme Godzilla com os seguintes valores: [78, 52, 95, 102].

db.movies.updateOne({'title': 'Godzilla'}, {$addToSet: {'ratings': { $each: [78, 52, 95, 102]}}});

//? 5 - Adicione um array chamado ratings ao filme Home Alone com os seguintes valores: [200, 99, 65].

db.movies.updateOne({'title': 'Home Alone'}, {$addToSet: {'ratings': { $each: [200, 99, 65]}}});
...
db.movies.find({$or: [{ 'ratings': { $elemMatch: { $gt: 199 }} }, {'category': 'sci-fi'} ]}, {_id: 0, 'title': 1, 'ratings': 1, 'category': 1});
